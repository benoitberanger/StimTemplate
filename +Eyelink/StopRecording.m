    ...
    if Eyelink('IsConnected')
        ...
        if err == 0 % 0 means recording
            ...
            if status ~= 0
                ...
                disp(['Eyelink closed file : ' DataStruct.EyelinkFile])
            end
            
            % Receive file
            disp('Eyelink file transfer IN PROGRESS')
            disp('It might take a few seconds...')
            status = Eyelink('ReceiveFile', DataStruct.EyelinkFile, DataStruct.DataPath, 1);
            if status > 0
                disp('Eyelink file transfer DONE')
                disp([ DataStruct.EyelinkFile ' size is ' num2str(status) ])
            elseif status == 0
            ...
            % Rename Eyelink file
            movefile([DataStruct.DataPath filesep DataStruct.EyelinkFile '.edf'], [DataStruct.DataFile '.edf'])
